...
> *译注：*
>
> 通过 `torrent_status::error` 变量获取错误信息是 Libtorrent ABI V1 接口中的用法。
> Libtorrent ABI V2 中，应该使用 `torrent_status::errc::message()` 来获取错误信息。
...
- 在 [torrent_handle](https://www.libtorrent.org/reference-Torrent_Handle.html#save_resume_data()) 上，
调用 [save_resume_data()](https://www.libtorrent.org/reference-Torrent_Handle.html#torrent_handle)，并确保传入 `save_info_dict` 标志。
- 等待 `resume_data_alert` 。
- 调用 [write_torrent_file()](https://www.libtorrent.org/reference-Resume_Data.html#write_torrent_file()) ，
并传入从 [alert](https://www.libtorrent.org/reference-Alerts.html#alert) 中
获取的 [add_torrent_params](https://www.libtorrent.org/reference-Add_Torrent.html#add_torrent_params)对象。
...
> *译注：*
>
> 混合种子文件同时包含 v1 和 v2 的元数据。例如，info 字典中既有 v1 的 pieces 字段（SHA-1 哈希列表），也有 v2 的 file tree 和 piece layers 字段（默克尔树结构）。
> 这时，在解析时发现已经存在了已解析的混合 torrent 冲突时，把后解析的 torrent 直接移除就行。
...
在已经达到做种目标的 `torrent` 中(已经达到了上传的数据量或达到了预设的上传比例的 `torrent`)，那些我们不知道有其他做种者的 `torrent` 将严格优先继续做种。
...
这意味着，只要没有选择任何数据块，多个 peers 的加入或离开的开销不会比单个 peer 的加入或离开更高。
...
> *译注：*
>
> 部分 Piece 被下载：piece 下载的时候可以指定第几个 Piece 的 offset 开始下载，下载多少字节数。所以，就有可能造成一些 Piece 只被下载了一部分。
...
> 译注：
>
> 0.0.0.0是 IPv4 中的未指定地址，::是 IPv6 中的未指定地址。当 libtorrent 遇到这样的未指定地址时，它会进行解析，去寻找并列举出所有符合对应地址族（IPv4 或 IPv6）的可用地址。
> 这就好比一个程序在一个大的地址空间里去寻找所有它能使用的具体地址

如果 IP 地址是未指定地址（即 `0.0.0.0` 或 `::`），libtorrent 会将其解析为具体的 IP 地址。此解析将枚举其能够找到的对应地址族的所有地址。
如果满足以下任一条件，解析后的 IP 地址将被视为“本地网络”：

- IP 地址位于已知的 `link-local` 范围内(同一个网关内，比如：在 IPv4 中 169.254.0.0/16 和在 IPv6 中 FE80::/10 开头的地址，就在链路本地地址范围内)
- IP 地址位于已知的 `loopback` 范围内(比如 IPv4 中的 127.0.0.1 和 IPv6 中的::1)
- 解析 IP 地址的原始项被标记为本地（`l`）
- 网络接口设置了`loopback` 标志
- 同时，以下条件均**不满足**：
  1. IP 地址位于全局可访问的 IP 地址范围内。
  
      以 IPv4 为例，其全局可访问的地址范围是除了私有地址范围（如 10.0.0.0 - 10.255.255.255、172.16.0.0 - 172.31.255.255、192.168.0.0 - 192.168.255.255）和其他特殊用途地址之外的地址空间。

  2. 网络接口设置了 `point-to-point` 标志
  3. 路由表中包含至少一个全局互联网地址（例如默认路由 0.0.0.0/0）的路由，该路由指向解析 IP 地址的网络接口，且地址族与解析 IP 地址的地址族一致。(可以参见附录一的说明)

### routing (路由)

如果满足以下条件中的任何一个，则认为 `listen_socket_t` 项能够路由到目标地址（只是认为，并不一定真的可以路由到目标地址）：

- 目标地址位于其子网内（即本机接口地址与网络掩码的按位与结果，与目标地址与网络掩码的按位与结果相同）。
...
- **计数器** 是一个单调递增的值，每次发生某个事件时都会增加。例如，每次网络线程因为套接字变为可读而唤醒时，计数器会增加。另一个例子是，每次套接字接收到 n 个字节时，计数器会增加 n。
  ...
  （不明白的话，可以看下 "附录二：基于速率的采样和基于计数器的采样" ）
...
在此 [alert](https://www.libtorrent.org/reference-Alerts.html#alert) 对象中，有一个值数组，这些值构成了样本。
`stats metric` 中的 `value_index` 字段值，指示了该指标的值存储在 `session_stats_alert` 数组中的哪个位置。
...
- **当前主机的 IP 地址**：假设当前主机的 IP 地址是 `192.168.1.50`，并且它通过 `eth0` 接口连接到局域网。
- **目标 IP 地址**：当前主机试图访问 `192.168.1.100`（同一局域网中的另一台设备）。
...
1. **默认路由**：
   - `0.0.0.0/0` 表示默认路由，指向网关 `192.168.1.1`，并通过 `eth0` 接口发送数据包。
   - 这意味着任何目标地址不在更具体路由中的流量都会通过 `eth0` 发送到 `192.168.1.1`，从而访问外部网络（如互联网）。

2. **本地网络路由**：
   - `192.168.1.0/24` 是一个本地网络路由，指向 `eth0` 接口。
     - 这意味着所有目标地址在 `192.168.1.0/24` 范围内的流量（如 `192.168.1.100`）都会通过 `eth0` 接口直接发送，而不需要经过网关。
   - `10.0.0.0/8` 是另一个本地网络路由，指向 `eth1` 接口。
     - 如果目标地址在 `10.0.0.0/8` 范围内，流量会通过 `eth1` 接口发送。

---

### 判断 `192.168.1.100` 是否是本地网络地址

根据你的特定路由条件：

- **条件 1**：路由表中包含至少一个全局互联网地址（如默认路由 `0.0.0.0/0`）。
  - 满足，因为路由表中有 `0.0.0.0/0` 指向 `192.168.1.1`。
- **条件 2**：该路由指向解析 IP 地址的网络接口（即 `192.168.1.100` 通过 `eth0` 接口通信）。
  - 满足，因为 `192.168.1.100` 属于 `192.168.1.0/24` 网络，通过 `eth0` 接口通信。
- **条件 3**：地址族与解析 IP 地址的地址族一致（都是 IPv4）。
  - 满足，因为 `192.168.1.100` 和默认路由都是 IPv4 地址。

---

### 结论

由于满足所有条件，`192.168.1.100` **不能被认为是仅用于本地网络通信的地址**。
这是因为 `eth0` 接口不仅有到 `192.168.1.100` 的本地路由，还有到全局互联网的默认路由。因此，`eth0` 接口既可以用于本地网络通信，也可以用于访问全局互联网。

## 附录二：基于速率的采样和基于计数器的采样
...
- **什么是速率采样？**
  - 速率采样是指定期（例如每秒）测量当前的下载速率（如 1 MB/s）。
  - 例如，每秒记录一次下载速率，得到一系列速率值：`[1 MB/s, 1.2 MB/s, 0.8 MB/s, ...]`。

- **如何转换为总传输量？**
  - 为了计算总传输量，需要将速率值乘以采样间隔时间，然后累加。
  - 例如，如果每秒采样一次，速率为 `[1 MB/s, 1.2 MB/s, 0.8 MB/s]`，则总传输量为：
    ```
    1 MB/s * 1s + 1.2 MB/s * 1s + 0.8 MB/s * 1s = 3 MB
    ```

- **为什么不准确？**
  - **采样间隔问题**：如果采样间隔较大（例如每秒一次），而实际速率在采样间隔内波动较大，则采样值无法准确反映真实速率。
    - 例如，如果在某次采样间隔内，速率从 1 MB/s 突然增加到 10 MB/s，然后又降回 1 MB/s，采样值可能只记录到 1 MB/s，导致总传输量计算偏低。
  - **需要更多样本**：为了减少误差，需要更频繁地采样（例如每毫秒一次），但这会增加系统开销。

#### 2. **基于计数器的方式**

- **什么是计数器？**
  - 计数器是一个单调递增的值，每次发生某个事件时都会增加。
  - 例如，每次接收到 `n` 字节数据时，计数器增加 `n`。
  - 例如，计数器值可能是：`[0, 1024, 2048, 3072, ...]`（单位：字节）。

- **如何转换为总传输量？**
  - 总传输量直接由计数器的当前值表示。
  - 例如，如果计数器当前值为 3072 字节，则总传输量为 3072 字节。

- **如何转换为速率？**
  - 速率可以通过计算计数器在某个时间间隔内的增量来得到。
  - 例如，如果在 1 秒内计数器从 1024 字节增加到 3072 字节，则速率为：
    ```
    (3072 - 1024) / 1s = 2048 字节/s
    ```

- **为什么简单且灵活？**
  - **准确性**：计数器直接记录总传输量，没有采样误差。
  - **灵活性**：可以根据需要选择任意时间间隔计算速率，而不受采样间隔的限制。
    - 例如，可以计算过去 1 秒、5 秒或 10 秒的平均速率。
  - **低开销**：计数器只需要在事件发生时更新，不需要定期采样。