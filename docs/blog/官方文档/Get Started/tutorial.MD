...
> 译注： 在 libtorrent `中，add_torrent_params` 是一个结构体或类，用于指定要添加的 `torrent` 的各种参数，比如 `torrent` 文件的路径、保存位置、下载模式等。通过填充这个对象，你可以告诉 libtorrent 你想要如何下载这个 `torrent` 。
> 
...
> 译注： 当你调用 `add_torrent()` 方法时，程序会等待这个 `torrent` 被成功添加到 `session` 中，然后才会继续执行后面的代码。这个过程是阻塞的，意味着在 `torrent` 被添加完成之前，程序不会继续往下运行。
> 
...
> 译注： `alert` 结构的子类包括 `torrent_alert` 、 `piece_finished_alert` 、 `peer_connect_alert` 等等，这些子类都是在 `alert_type.hpp` 文件中定义的。
...
> 译注： 在 BitTorrent 协议中，当你想要停止一个 torrent （即一个正在通过 BitTorrent 协议下载或上传的文件）时，你的 BitTorrent 客户端（比如 libtorrent ）需要与 tracker 服务器进行联系。
>
...
> 译注： 在 libtorrent 中， `session::abort()` 方法允许异步地停止和销毁 `session` 对象。调用 `abort()` 后， `session` 会立即开始关闭过程，但不会等待所有操作完成。这意味着在 `abort()` 被调用后， `session` 可能会继续处理一些未完成的操作（如与 tracker 的通信），但这些操作会在后台进行，不会阻塞您的程序。
>
...
> 译注： 当您决定关闭一个 `session` 时，可能需要一个机制来确保在关闭过程中 `session` 的状态仍然可以被访问或查询，但又不允许进行任何会修改 `session` 状态的操作。这就是 `session_proxy` 对象的作用。它是一个轻量级的句柄，允许您在关闭 session 的过程中只能以只读方式访问其状态。
...
> 译注： 
>  - “提高可扩展性至关重要”的意思应该指的是，如果每个 torrent 的状态变化都立即触发处理，那么可能会给系统带来很大的负担。而 state_update_alert 的这个特性允许系统批量处理只在一段时间内状态发生变化的 torrent，从而减轻了系统的负担，提高了整体的可扩展性和性能。4
>  - 状态变化的检测：
libtorrent 会跟踪每个 torrent 的状态变化，包括下载进度、上传进度、连接数等。
即使 torrent 的总体状态（如 downloading）没有变化，只要其内部状态（如 total_download）发生了变化，libtorrent 就会将其标记为“状态已更新”。
（所以，可以设计成下载时划分固定的时间间隔，将该时间间隔内的下载字节数保存到以时间为序列的 slot 中，然后就可以计算这个时间序列的速度，而不需要实时的每次有数据下载时就计算下载速度）
...
1. 从磁盘上读取下载文件的每一块 `piece` ，并将其与预期的哈希值进行比较 (译注： 每一块 `piece` 的校验码都是在 `torrent` 的 `info` 字段中存放的, 下载文件中的数据转换 `hash` 和 `info` 字段中对应的 `piece hash` 进行对比，如果不一致，则说明没下载或下载的内容出错了)。
2. 将已下载文件部分（以及部分下载的文件块）的状态信息保存到磁盘上，并在恢复下载时重新加载这些信息。

如果添加的 `torrent` 没有提供恢复数据， libtorrent 将默认采用第 1 种方式。

要保存恢复的数据，请在 `torrent_handle` 对象上调用 [save_resume_data()](https://www.libtorrent.org/reference-Torrent_Handle.html#save_resume_data()) 函数。这将请求 libtorrent 生成恢复数据(填充 DTO)，并在一个 [save_resume_data_alert](https://www.libtorrent.org/reference-Alerts.html#save_resume_data_alert) 中将其返回。如果出于任何原因生成恢复数据失败，则会发布一个 [save_resume_data_failed_alert](https://www.libtorrent.org/reference-Alerts.html#save_resume_data_failed_alert) 。


每当调用 [save_resume_data()](https://www.libtorrent.org/reference-Torrent_Handle.html#save_resume_data()) 函数时，都会准确地发布一个 `alert` 消息。这在关闭包含多个 `torrent` 的 `session` 时是一个重要的特性，因为在恢复关闭着的 `session` 之前，必须处理每一个 `resume alert` (译注：意思是没有在关闭 `session` 时保存好每一个 `resume alert` 的数据 ，那么之后就无法恢复整个 `session` 的 `torrent` ) 。任何 torrent 都可能无法保存恢复数据，因此客户端需要记录尚未保存的 resume 文件的数量，这个数量在收到 [save_resume_data_alert](https://www.libtorrent.org/reference-Alerts.html#save_resume_data_alert) 或 [save_resume_data_failed_alert](https://www.libtorrent.org/reference-Alerts.html#save_resume_data_failed_alert) 时都会相应减少（译注：等到该数量为 0  时，说明所有的 torrent resume data 都保存了，可以安全退出了）。
...
`params` 字段是一个 [add_torrent_params](https://www.libtorrent.org/reference-Add_Torrent.html#add_torrent_params) 对象，它包含了将 `torrent` 加回 session 中时所需的所有状态。这个对象可以使用 [write_resume_data()](https://www.libtorrent.org/reference-Resume_Data.html#write_resume_data()) 或 [write_resume_data_buf()](https://www.libtorrent.org/reference-Resume_Data.html#write_resume_data_buf()) 方法进行序列化，对数据进行 bencoded 编码。用 [read_resume_data()](https://www.libtorrent.org/reference-Resume_Data.html#read_resume_data()) 方法可以进行反序列化。

> 译注：
> 
> 实际上 write_resume_data(atp) 方法是在 atp 基础上，添加了些内容，生成用于 beconding 的 entry 对象，也就是 resume_data 对象，它还没进行 bencoded 编码。
> write_resume_data_buf(entry) 才是用来对 entry 进行 bencoded 编码的。
  ...
  for (;;) {
    ...
    for (lt::alert const* a : alerts) {
      ...
      if (auto rd = lt::alert_cast<lt::save_resume_data_alert>(a)) {
        ...
        // 将 rd-> params 这个 DTO 进行 bencoded 编码保存到 b 中
    ...
    // 注意：这条语句导致最终只有状态有变化的 torrent 才会被保存 resume_data。
...
> 译注： `session_params` 中的 `disk_io_constructor` 成员属性，其实是一个函数对象，调用它会生成一个 **用来指定磁盘 I/O 的后端实现** 的 `unique_ptr<disk_interface>` 对象。
